...
int put_codepoint(char const* utf8_codepoint, size_t length, int expectedWidth)
{
#if defined(_WIN32)
    CONSOLE_SCREEN_BUFFER_INFO bufferInfo;
...
bool readline_advanced(std::string& line, bool multiline_input)
{
    ...
    while (true) {
        ...
        if (input_char == '\033') {  // Escape sequence
            ...
            int width = put_codepoint(line.c_str() + offset, line.length() - offset,
                                      estimateWidth(input_char));
            if (width < 0) {
                width = 0;
            }
...
bool readline_simple(std::string& line, bool multiline_input)
{
    ...
    int size_needed =
        WideCharToMultiByte(CP_UTF8, 0, &wline[0], (int)wline.size(), NULL, 0, NULL, NULL);
    line.resize(size_needed);
    WideCharToMultiByte(CP_UTF8, 0, &wline[0], (int)wline.size(), &line[0], size_needed, NULL,
                        NULL);
#else
