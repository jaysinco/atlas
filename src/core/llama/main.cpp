...
void write_logfile(llama_context const* ctx, gpt_params const& params, llama_model const* model,
                   std::vector<llama_token> const& input_tokens, std::string const& output,
                   std::vector<llama_token> const& output_tokens)
{
    ...
    bool const success = create_directory_with_parents(params.logdir);
    if (!success) {
...
void sigint_handler(int signo)
{
    if (signo == SIGINT) {
        if (!is_interacting) {
            ...
            write_logfile(*g_ctx, *g_params, *g_model, *g_input_tokens, g_output_ss->str(),
                          *g_output_tokens);
            _exit(130);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    int const n_ctx_train = llama_n_ctx_train(ctx);
    ...
    // determine the maximum memory usage needed to do inference for the given n_batch and n_ctx
    // parameters uncomment the "used_mem" line in llama.cpp to see the results
    if (params.mem_test) {
        {
    ...
    if (!path_session.empty()) {
        ...
        if (fp != NULL) {
            ...
            if (!llama_load_session_file(ctx, path_session.c_str(), session_tokens.data(),
    ...
    bool const add_bos = llama_vocab_type(ctx) == LLAMA_VOCAB_TYPE_SPM;
    LOG("add_bos: %d\n", add_bos);
    ...
    if (!session_tokens.empty()) {
        ...
        if (params.prompt.empty() && n_matching_session_tokens == embd_inp.size()) {
            ...
            LOG_TEE(
                "%s: warning: session file has low similarity to prompt (%zu / %zu tokens); will "
                "mostly be reevaluated\n",
    ...
    LOGLN(
        "recalculate the cached logits (check): embd_inp.empty() %s, n_matching_session_tokens "
        "%zu, embd_inp.size() %zu, session_tokens.size() %zu, embd_inp.size() %zu",
    ...
    auto const inp_pfx = ::llama_tokenize(ctx, "\n\n### Instruction:\n\n", add_bos);
    auto const inp_sfx = ::llama_tokenize(ctx, "\n\n### Response:\n\n", false);
    ...
    if (params.interactive) {
        ...
        if (!params.antiprompt.empty()) {
            for (auto const& antiprompt: params.antiprompt) {
    ...
    LOG_TEE(
        ...
        "frequency_penalty = %f, top_k = %d, tfs_z = %f, top_p = %f, typical_p = %f, temp = %f, "
    ...
    if (!params.grammar.empty()) {
        ...
        std::vector<llama_grammar_element const*> grammar_rules(parsed_grammar.c_rules());
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (!embd.empty()) {
            ...
            if ((int)embd.size() > max_embd_size) {
                int const skipped_tokens = (int)embd.size() - max_embd_size;
                embd.resize(max_embd_size);

                console::set_display(console::error);
                printf("<<input too long: skipped %d token%s>>", skipped_tokens,
                       skipped_tokens != 1 ? "s" : "");
            ...
            if (n_past + (int)embd.size() + std::max<int>(0, guidance_offset) > n_ctx) {
                ...
                int const n_left = n_past - params.n_keep;
                ...
                embd.insert(embd.begin(), last_tokens.begin() + n_ctx - n_left / 2 - embd.size(),
                            last_tokens.end() - embd.size());
            ...
            // try to reuse a matching prefix from the loaded session instead of re-eval (via
            // n_past)
                ...
                if (n_past_guidance < (int)guidance_inp.size()) {
                    ...
                    if (embd.begin() + original_prompt_len < embd.end()) {
                        embd_guidance.insert(embd_guidance.end(),
                                             embd.begin() + original_prompt_len, embd.end());
                    }
        ...
        if ((int)embd_inp.size() <= n_consumed && !is_interacting) {
            ...
            const llama_token id =
                llama_sample_token(ctx, ctx_guidance, grammar, params, last_tokens, candidates);
        ...
        if ((int)embd_inp.size() <= n_consumed) {
            ...
            if (!params.antiprompt.empty()) {
                ...
                // If we're not running interactively, the reverse prompt might be tokenized with
                // some following characters so we'll compensate for that by widening the search
                // window a bit.
            ...
            if (n_past > 0 && is_interacting) {
                ...
                if (buffer.length() > 1) {
                    ...
                    auto const line_inp = ::llama_tokenize(ctx, buffer, false);
            ...
            if (n_past > 0) {
                if (is_interacting) {
                    ...
                    if (grammar != NULL) {
                        ...
                        std::vector<llama_grammar_element const*> grammar_rules(
        ...
        // In interactive mode, respect the maximum number of tokens and drop back to user input
        // when reached. We skip this logic when n_predict == -1 (infinite) or -2 (stop at context
        // size).
