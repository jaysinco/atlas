...
std::shared_ptr<Player> createPlayer(std::string const& setup)
{
    ...
    static std::regex pattern(R"(i(\d+)u([\d.]+)t(\d+)(?:@(\d+)g?)?)");
    int itermax;
    float c_puct;
    int nthreads;
    ...
    bool use_gpu = setup.back() == 'g';
    ILOG("itermax={}, c_puct={}, nthreads={}{}", itermax, c_puct, nthreads,
         verno ? FSTR(", verno={}, use_gpu={}", *verno, use_gpu) : "");
...
int main(int argc, char** argv)
{
    ...
    train_args.optional("gpu,g", po::bool_switch()->default_value(false), "use gpu");
    ...
    play_args.positional("player1", po::value<std::string>(),
                         "player setup like i1280u2.4t16@1g, human", 1);
